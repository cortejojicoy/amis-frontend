<template>
  <div class="my-6">
    <div class="flex justify-between mb-6">
      <div>
        <button @click="addRemoveMentor" class="p-2 bg-gray-300 btn" :disabled="isNominatedMentorsLoading">Add/Remove a Mentor</button>
    ...
    <div class="bg-white overflow-auto shadow-xl sm:rounded-lg mb-4">
      ...
      <td>
        ...
        <select  
          ...
          @change="updateMentorRecord('actions',record.id, $event)"
        >
          <option
      ...
      <td v-if="record.actions === 'Add'">
         <select 
          :value="record.mentor_id"
          @change="updateMentorRecord('mentor_id',record.id, $event)"
         >
          <option></option>
          <option
            v-for="(mentor, mentorIndex) in facultyNames"
            :value="mentor.saisid"
            :key="mentorIndex"
          >
            {{mentor.mentor_name}}
      ...
      <td>
        <select :value="record.mentor_role"
         @change="updateMentorRecord('mentor_role',record.id, $event)"
         >
          <option
            v-for="(mentorRole, mentorRoleIndex) in roleList"
            :key="mentorRoleIndex"
          >
            {{mentorRole}}
          </option>
      ...
      <div>
        <button class="p-2 bg-gray-200 mr-2" @click="clickSaveMentor" :disabled="isNominatedMentorsLoading">Save</button>
      </div>
      <div>
